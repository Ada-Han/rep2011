...
def build_modal_spatial_moments(coarse_solution, reference_solution):
    ...
    # 预计算积分点及 VEM 投影器
    coarse_model.precompute_quadrature_points()

    # 批量提取顶点数值并投影
    coarse_local = coarse_solution["mode_vector"][coarse_model.elements]
    ref_local = reference_mode_vector_restricted[coarse_model.elements]

    coeffs_coarse = np.einsum('eij,ej->ei', coarse_model.Pi_star_all, coarse_local)
    coeffs_ref = np.einsum('eij,ej->ei', coarse_model.Pi_star_all, ref_local)

    # 批量求积分点处的数值解
    coarse_quad_vals = np.einsum('eki,ei->ek', coarse_model.quad_m_bases, coeffs_coarse)
    ref_quad_vals = np.einsum('eki,ei->ek', coarse_model.quad_m_bases, coeffs_ref)

    # 计算积分值
    domain_measure = np.sum(coarse_model.quad_weights)
    coarse_mode_mean = np.sum(coarse_quad_vals * coarse_model.quad_weights)
    reference_mode_mean = np.sum(ref_quad_vals * coarse_model.quad_weights)
    coarse_mode_square = np.sum((coarse_quad_vals ** 2) * coarse_model.quad_weights)
    reference_mode_square = np.sum((ref_quad_vals ** 2) * coarse_model.quad_weights)
    cross_mode_product = np.sum((coarse_quad_vals * ref_quad_vals) * coarse_model.quad_weights)

    return {
        "domain_measure": domain_measure,
        "coarse_mode_mean": coarse_mode_mean,
        "reference_mode_mean": reference_mode_mean,
        "coarse_mode_square": coarse_mode_square,
        "reference_mode_square": reference_mode_square,
        "cross_mode_product": cross_mode_product,
    }
...
def print_spatial_error_table(results):
    """打印第二组实验的空间误差表。"""
    print("2D Group-2 Vectorized Experiment")
    print("=" * 96)
...
def plot_group2_results(reference_solution, finest_solution, results):
    ...
    plt.title("Group 2: spatial convergence on the 2D baseline (vectorized)")
