...
def build_age_grid(dt, a_max=A_MAX):
    """构造年龄网格，delta a = delta t 时间步长相同，沿着特征线。"""
...
def build_modal_spatial_moments(coarse_solution, reference_solution):
    ...
    quadrature = coarse_model.get_quadrature()
    ...
    for element in coarse_model.elements:
        ...
        _, _, abs_det_jacobian = coarse_model.calculate_physical_derivatives(0.0, 0.0, node_coords)
        ...
        for xi, eta, weight in quadrature:
            shape_values = coarse_model.p1_shape_functions(xi, eta)
            quadrature_weight = abs_det_jacobian * weight

            coarse_value = float(np.dot(shape_values, coarse_local_values))
            reference_value = float(np.dot(shape_values, reference_local_values))
